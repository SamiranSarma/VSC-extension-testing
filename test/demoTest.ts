...
let { ActivityBar, ExtensionsViewItem, ExtensionsViewSection } = require('vscode-extension-tester') ;
...
describe('Example extension view tests', () => {
	...
	before(async function () {
		...
		const extensions = (await view?.getContent().getSection('Installed')) ;
		...
		await extensions.getDriver().wait(async function () {
			helloExtension = (await extensions.findItem(`@installed ${pjson.displayName}`));
	...
	it('Check the extension info', async () => {
		...
		console.log("The Actual extension publisher is :"+author+", description is: "+desc+" and the extension version is: "+version)
